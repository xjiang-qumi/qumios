...
typedef struct
{
    yopen_uart_config_s usart_config;
...
extern const yopen_uart_func_s yopen_uart_pin_func[];
__attribute__((used)) static bool config_uart_bit_func(int uartPort)
{
    ...
    if(yopen_uart_pin_func[uartPort].port == YOPEN_PORT_NONE)
        return false;

    uart_tx_bit = yopen_uart_pin_func[uartPort].tx_pin;
    uart_tx_func = yopen_uart_pin_func[uartPort].tx_func;
    uart_rx_bit = yopen_uart_pin_func[uartPort].rx_pin;
    uart_rx_func = yopen_uart_pin_func[uartPort].rx_func;

    if(uart_tx_bit == YOPEN_UART_PIN_NONE || uart_tx_func != YOPEN_UART_PIN_NONE)
        return false;

    yopen_pin_set_func(uart_tx_bit, uart_tx_func);
    yopen_pin_set_func(uart_rx_bit, uart_rx_func);

    return true;
}

static void yopen_uart_notify_cb(uint32 ind_type, yopen_uart_port_number_e port, uint32 size)
{
    ...
    switch (ind_type)
    {
        case YOPEN_UART_EVENT_RX_OVERFLOW: // rx buffer overflow
        case YOPEN_UART_EVENT_RX_ARRIVED:
        {
            ...
            while (size > 0)
            {
                ...
                read_len  = yopen_uart_read(port, recv_buf, real_size);
        ...
        case YOPEN_UART_EVENT_TX_COMPLETE:
        {
            break;
        }
    }
...
int32_t qm_uart_init(qm_uart_dev_t *uart)
{
    ...
    if(uart->config.data_bits == QM_UART_DATA_8_BITS){
        info->usart_config.data_bit = YOPEN_UART_DATABIT_8;
    }else if(uart->config.data_bits == QM_UART_DATA_7_BITS){
        info->usart_config.data_bit = YOPEN_UART_DATABIT_7;
    }else{
        return -QM_ERROR;
    }

    if(uart->config.parity == QM_UART_PARITY_NONE){
        info->usart_config.parity_bit = YOPEN_UART_PARITY_NONE;
    }else if(uart->config.parity == QM_UART_PARITY_EVEN){
        info->usart_config.parity_bit = YOPEN_UART_PARITY_EVEN;
    }else if(uart->config.parity == QM_UART_PARITY_ODD){
        info->usart_config.parity_bit = YOPEN_UART_PARITY_ODD;
    }

    if(uart->config.stop_bits == QM_UART_STOP_BITS_1){
        info->usart_config.stop_bit = YOPEN_UART_STOP_1;
    }else if(uart->config.stop_bits == QM_UART_STOP_BITS_2){
        info->usart_config.stop_bit = YOPEN_UART_STOP_2;
    }else{
        return -QM_ERROR;
    }

    if(uart->config.flow_ctrl == QM_UART_FLOW_CTRL_NONE){
        info->usart_config.flow_ctrl = YOPEN_FC_NONE;
    }else if(uart->config.flow_ctrl == QM_UART_FLOW_CTRL_CTS_RTS){
        info->usart_config.flow_ctrl = YOPEN_FC_HW;
    }else{
        return -QM_ERROR;
    }
    ...
    yopen_uart_set_dcbconfig(uart->port, &info->usart_config);

    ret = yopen_uart_open(uart->port);
    if (ret == YOPEN_UART_SUCCESS){
        yopen_uart_register_cb(uart->port, yopen_uart_notify_cb);
        QM_LOGE("uart", "uart %d open success", uart->port);
    }else{
        QM_LOGE("uart", "uart %d open fail", uart->port);
        return -QM_ERROR;
    }
    yopen_uart_tx_way_config(YOPEN_UART_TX_DRIVER);
...
int32_t qm_uart_write(qm_uart_dev_t *uart, const void *data, uint32_t size, uint32_t timeout)
{
    ...
    return yopen_uart_write(uart->port, (uint8_t*)data, size);
}
