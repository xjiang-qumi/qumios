...
void *qm_tls_establish(const char *host, uint16_t port, const qm_tls_cfg_t *cfg)
{
    ...
    /* 检查是否需要进行认证 */
    int do_auth = 1;
    if (cfg->ca_crt == NULL || cfg->ca_crt[0] == '\0' ||
        cfg->client_crt == NULL || cfg->client_crt[0] == '\0' ||
        cfg->client_key == NULL || cfg->client_key[0] == '\0') {
        do_auth = 0;
        QM_LOGW(TAG, "CA cert, client cert or client key is empty, skip authentication");
    }
