...
static const int PPP_FORCE_RESET_BIT              = BIT9;    /* force reset, trigger by daemon task, clear by daemon task */
...
typedef enum {
    ...
    STAGE_CHECK_GPSINFO = 6, /* check is gpsinfo to operator */
    STAGE_START_PPP = 7,    /* trying dial-up */
    STAGE_WAIT_IP = 8,      /* waiting ip */
    STAGE_RUNNING = 9,      /* perfect, enjoy network */
} _modem_stage_t;
...
static bool _check_lbs_info()
{
    float longitude;
    float latitude;
    if (modem_board_get_lbs(&longitude, &latitude) == ESP_OK) {
        ESP_LOGI(TAG, " lbs info: longitude=%.7f, latitude=%.7f", longitude, latitude);
        return true; 
    }
    ESP_LOGW(TAG, "Get signal quality failed");
    return false;
...
esp_err_t modem_board_get_gpsinfo(void);
...
static void _modem_daemon_task(void *param)
{
    ...
    while (true) {
        ...
        EventBits_t bits = xEventGroupWaitBits(s_modem_evt_hdl, ( PPP_NET_MODE_ON_BIT | PPP_NET_MODE_OFF_BIT | DTE_USB_RECONNECT_BIT | DTE_USB_DISCONNECT_BIT | PPP_NET_RECONNECTING_BIT |
                                               PPP_NET_DISCONNECT_BIT | MODEM_DESTROY_BIT | PPP_FORCE_RESET_BIT), pdFALSE, pdFALSE, portMAX_DELAY);
        ...
        if(bits & PPP_FORCE_RESET_BIT){
            xEventGroupClearBits(s_modem_evt_hdl, (PPP_NET_MODE_OFF_BIT | MODEM_DESTROY_BIT | DTE_USB_RECONNECT_BIT | PPP_NET_RECONNECTING_BIT | PPP_FORCE_RESET_BIT));
            modem_board_force_reset();
            esp_event_post(MODEM_BOARD_EVENT, MODEM_EVENT_DTE_RESTART_DONE, NULL, 0, 0);
        ...
        case STAGE_CHECK_REGIST:
            if (_check_network_registration() != true)  {
                ...
                modem_stage = STAGE_CHECK_GPSINFO;
                goto _stage_succeed;
            }
            break;
        
        case STAGE_CHECK_GPSINFO:
            if (_check_lbs_info() != true)  {
                retry_after_ms = 3000;
                ++stage_retry_times;
            } else {
...
esp_err_t modem_board_ppp_restart(uint32_t timeout_ms)
{
    xEventGroupSetBits(s_modem_evt_hdl, PPP_FORCE_RESET_BIT);
    modem_board_ppp_start(timeout_ms);
    return ESP_OK;
}
...
esp_err_t modem_board_get_signal_quality(int *rssi, int *ber)
{
    ...
    if (err == ESP_OK) {
        ...
        esp_event_post(MODEM_BOARD_EVENT, MODEM_EVENT_CSQ, &result, sizeof(esp_modem_dce_csq_ctx_t), 0);
    }
    return err;
}

esp_err_t modem_board_get_lbs(float *longitude, float *latitude)
{
    MODEM_CHECK(s_dce != NULL, "modem not ready", ESP_ERR_INVALID_STATE);
    esp_modem_dce_lbs_ctx_t result;
    esp_err_t err = esp_modem_dce_get_lbs(s_dce, NULL, &result);
    if (err == ESP_OK) {
        if(longitude) *longitude = result.longitude;
        if(latitude) *latitude = result.latitude;
        esp_event_post(MODEM_BOARD_EVENT, MODEM_EVENT_LBS, &result, sizeof(esp_modem_dce_lbs_ctx_t), 0);
    }
    return err;
}
