...
/**
 * @brief Handle response from AT+CGPSINFO
 */
static esp_err_t esp_modem_dce_common_handle_lbs(esp_modem_dce_t *dce, const char *line)
{
    esp_err_t err = ESP_FAIL;
    if (strstr(line, MODEM_RESULT_CODE_ERROR)) {
        err = esp_modem_process_command_done(dce, ESP_MODEM_STATE_FAIL);
    ...
    if (strstr(line, "+CIPGSMLOC")) {
        /* store value of rssi and ber */
        esp_modem_dce_lbs_ctx_t *lbs = dce->handle_line_ctx;
        /* +CSQ: <rssi>,<ber> */
        sscanf(line, "%*[^,],%f,%f", &lbs->longitude, &lbs->latitude);
        err = ESP_OK;
    }
...
esp_err_t esp_modem_dce_get_lbs(esp_modem_dce_t *dce, void *param, void *result)
{
    return esp_modem_dce_generic_command(dce, "AT+CIPGSMLOC=1,1\r", MODEM_COMMAND_TIMEOUT_DEFAULT,
                                         esp_modem_dce_common_handle_lbs, result);
}
