#include "qm_code128.h"
#include "qm_kernel.h"
#include "qm_log.h"

#if QM_BARCODE_SUPPORT

#define CODE128_MALLOC      qm_malloc
#define CODE128_REALLOC     qm_realloc
#define CODE128_FREE        qm_free
#define CODE128_MEMSET      memset
#define CODE128_STRLEN      strlen
#define CODE128_ASSERT      QM_ASSERT
...
uint32_t qm_code128_encode_raw(const char * s, char * out, uint32_t maxlength)
{
    ...
    const uint32_t overhead = CODE128_QUIET_ZONE_LEN
                            + CODE128_CHAR_LEN // checksum
                            + CODE128_STOP_CODE_LEN
                            + CODE128_QUIET_ZONE_LEN;
    ...
    uint32_t num_codes = state.maxlength / CODE128_CHAR_LEN;
    ...
    uint32_t actual_length = state.maxlength + overhead;
...
{
    uint32_t raw_size = CODE128_STRLEN(s) + 1;
    ...
    *p = '\0';
    ...
    uint32_t length = qm_code128_encode_raw(raw, out, maxlength);
