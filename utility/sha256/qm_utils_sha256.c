...
#endif
static void qm_utils_sha256_zeroize(void *v, size_t n)
{
...
{
    memset(ctx, 0, sizeof(qm_sha256_context));
}
void qm_utils_sha256_free(qm_sha256_context *ctx)
{
    ...
    qm_utils_sha256_zeroize(ctx, sizeof(qm_sha256_context));
}
...
void qm_utils_sha256_update(qm_sha256_context *ctx, const unsigned char *input, uint32_t ilen)
{
    ...
    /* Sanity check: */
    if (ctx == (qm_sha256_context *) 0 || input == (unsigned char *) 0) {
        return;
    }
    ...
    if (usedspace > 0) {
        ...
        if (ilen >= freespace) {
            ...
            ilen -= freespace;
            input += freespace;
            qm_utils_sha256_process(ctx, (uint32_t *) ctx->buffer);
        } else {
    ...
    while (ilen >= SHA256_BLOCK_LENGTH) {
        ...
        qm_utils_sha256_process(ctx, (uint32_t *) input);
...
{
    qm_sha256_context ctx;

    qm_utils_sha256_init(&ctx);
    qm_utils_sha256_starts(&ctx);
    qm_utils_sha256_update(&ctx, input, ilen);
    qm_utils_sha256_finish(&ctx, output);
    qm_utils_sha256_free(&ctx);
}
