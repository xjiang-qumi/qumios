...
#ifndef CONFIG_QM_JSON_DEBUG
#define CONFIG_QM_JSON_DEBUG  0
#endif
...
char *qm_json_get_next_object(int type, char *str, char **key, int *key_len, char **val, int *val_len, int *val_type);
...
#define json_object_for_each_kv(str, pos, key, klen, val, vlen, vtype) \
    for (pos = qm_json_get_object(JOBJECT, str); \
         pos!=0 && *pos!=0 && (pos=qm_json_get_next_object(JOBJECT, pos, &key, &klen, &val, &vlen, &vtype))!=0; )
...
#define json_array_for_each_entry(str, pos, entry, len, type) \
    for (pos = qm_json_get_object(JARRAY, str); \
         pos!=0 && *pos!=0 && (pos=qm_json_get_next_object(JARRAY, ++pos, 0, 0, &entry, &len, &type))!=0; )
