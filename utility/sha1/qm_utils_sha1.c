...
static void qm_utils_sha1_zeroize(void *v, size_t n)
{
...
{
    memset(ctx, 0, sizeof(qm_sha1_context));
}
...
void qm_utils_sha1_free(qm_sha1_context *ctx)
{
    ...
    qm_utils_sha1_zeroize(ctx, sizeof(qm_sha1_context));
}
...
void qm_utils_sha1_update(qm_sha1_context *ctx, const unsigned char *input, size_t ilen)
{
    ...
    if (left && ilen >= fill) {
        ...
        qm_utils_sha1_process(ctx, ctx->buffer);
        input += fill;
        ilen  -= fill;
        left = 0;
    }
    ...
    while (ilen >= 64) {
        qm_utils_sha1_process(ctx, input);
        input += 64;
        ilen  -= 64;
    }
...
void qm_utils_sha1_finish(qm_sha1_context *ctx, unsigned char output[20])
{
    ...
    qm_utils_sha1_update(ctx, iot_sha1_padding, padn);
    qm_utils_sha1_update(ctx, msglen, 8);
...
void qm_utils_sha1(const unsigned char *input, size_t ilen, unsigned char output[20])
{
    ...
    qm_utils_sha1_init(&ctx);
    qm_utils_sha1_starts(&ctx);
    qm_utils_sha1_update(&ctx, input, ilen);
    qm_utils_sha1_finish(&ctx, output);
    qm_utils_sha1_free(&ctx);
}
