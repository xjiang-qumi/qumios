...
int32_t qm_utils_aes_setkey_dec( qm_aes_context *ctx, uint8_t *key, int32_t keysize )
{
    ...
    ret = qm_utils_aes_setkey_enc( &cty, key, keysize );
    if( ret != 0 )
        return( ret );
    ...
    *RK++ = *SK++;
    ...
    memset( &cty, 0, sizeof( qm_aes_context ) );
    return( 0 );
}
                    ...
                    uint8_t *output )
{
    ...
    if( mode == AES_DECRYPT )
    {
        while( length > 0 )
        {
            ...
            qm_utils_aes_crypt_ecb( ctx, mode, input, output );
    ...
    else
    {
        while( length > 0 )
        {
            ...
            qm_utils_aes_crypt_ecb( ctx, mode, output, output );
            memcpy( iv, output, 16 );
                       ...
                       uint8_t *output )
{
    ...
    if( mode == AES_DECRYPT )
    {
        ...
        {
            if( n == 0 )
                qm_utils_aes_crypt_ecb( ctx, AES_ENCRYPT, iv, iv );
...
int32_t aes_self_test( int32_t verbose )
{
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        if( v == AES_DECRYPT )
        {
            qm_utils_aes_setkey_dec( &ctx, key, 128 + u * 64 );

            for( j = 0; j < 10000; j++ )
                qm_utils_aes_crypt_ecb( &ctx, v, buf, buf );
        ...
        {
            qm_utils_aes_setkey_enc( &ctx, key, 128 + u * 64 );
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        if( v == AES_DECRYPT )
        {
            ...
            for( j = 0; j < 10000; j++ )
                qm_utils_aes_crypt_cbc( &ctx, v, 16, iv, buf, buf );
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        if( v == AES_DECRYPT )
        {
            ...
            qm_utils_aes_crypt_cfb128( &ctx, v, 64, &offset, iv, buf, buf );
