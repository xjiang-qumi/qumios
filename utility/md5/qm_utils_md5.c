...
{
    memset(ctx, 0, sizeof(qm_md5_context));
}
...
void qm_utils_md5_free(qm_md5_context *ctx)
{
    ...
    utils_md5_zeroize(ctx, sizeof(qm_md5_context));
}
...
void qm_utils_md5_update(qm_md5_context *ctx, const unsigned char *input, size_t ilen)
{
    ...
    if (left && ilen >= fill) {
        ...
        qm_utils_md5_process(ctx, ctx->buffer);
        input += fill;
        ilen  -= fill;
        left = 0;
    }
    ...
    while (ilen >= 64) {
        qm_utils_md5_process(ctx, input);
        input += 64;
        ilen  -= 64;
    }
...
void qm_utils_md5_finish(qm_md5_context *ctx, unsigned char output[16])
{
    ...
    qm_utils_md5_update(ctx, iot_md5_padding, padn);
    qm_utils_md5_update(ctx, msglen, 8);
...
void qm_utils_md5(const unsigned char *input, size_t ilen, unsigned char output[16])
{
    ...
    qm_utils_md5_init(&ctx);
    qm_utils_md5_starts(&ctx);
    qm_utils_md5_update(&ctx, input, ilen);
    qm_utils_md5_finish(&ctx, output);
    qm_utils_md5_free(&ctx);
}
