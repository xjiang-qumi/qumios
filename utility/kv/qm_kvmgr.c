...
typedef enum {
    RES_OK              = 0,        /* Successed */
    RES_CONT            = -11,  /* Loop continued */
    RES_NO_SPACE        = -28,  /* The space is out of range */
    RES_INVALID_PARAM   = -22,  /* The parameter is invalid */
    RES_MALLOC_FAILED   = -12,  /* Error related to malloc */
    RES_ITEM_NOT_FOUND  = -2,  /* Could not find the key-value item */
    RES_FLASH_READ_ERR  = -5,     /* The flash read operation failed */
    RES_FLASH_WRITE_ERR = -5,     /* The flash write operation failed */
    RES_FLASH_EARSE_ERR = -5      /* The flash earse operation failed */
} result_e;
...
#define KV_PARAM_START_ADDRESS  (0xFA000)
#define KV_PARAM_START_SECTOR    (KV_PARAM_START_ADDRESS/SPI_FLASH_SEC_SIZE)

static uint32_t start_addr=KV_PARAM_START_ADDRESS;
static int raw_read(uint32_t offset, void *buf, size_t nbytes)
{
    //return hal_flash_read((hal_partition_t)KV_PTN, &offset, buf, nbytes);
    //printf("read offset %d, size %d\r\n",offset, nbytes);
    spi_flash_read(KV_PARAM_START_ADDRESS+offset, (uint32_t*)buf, nbytes);
    return 0;
}
...
static int raw_write(uint32_t offset, const void *buf, size_t nbytes)
{
	...
	spi_flash_write(KV_PARAM_START_ADDRESS+offset, (uint32_t*)buf, nbytes);
...
static int raw_erase(uint32_t offset, uint32_t size)
{
    ...
    //printf("erase offset %d, size %d\r\n",offset, size);
	spi_flash_erase_sector(KV_PARAM_START_SECTOR+offset/SPI_FLASH_SEC_SIZE);
...
static void trigger_gc(void)
{
    ...
    qm_task_new("kv-gc", qm_kv_gc, NULL, KV_GC_STACK_SIZE);
}
...
static int kv_item_del(kv_item_t *item, int mode)
{
    ...
    else if (mode == KV_ORIG_REMOVE)
    {
        ...
        origin_key = (char *)qm_malloc(hdr.key_len);
        if (!origin_key)
        {
            return RES_MALLOC_FAILED;
        }
        new_key = (char *)qm_malloc(hdr.key_len);
        if (!new_key)
        {
            qm_free(origin_key);
            return RES_MALLOC_FAILED;
...
{
    char *p = (char *)qm_malloc(item->len);
    if (!p)
    {
        return RES_MALLOC_FAILED;
    }
...
static int __item_find_cb(kv_item_t *item, const char *key)
{
    ...
    item->store = (char *)qm_malloc(item->hdr.key_len + item->hdr.val_len);
    if (!item->store)
    {
        return RES_MALLOC_FAILED;
    }
...
static int __item_del_by_prefix_cb(kv_item_t *item, const char *prefix)
{
    ...
    key = (char *)qm_malloc(item->hdr.key_len + 1);
    if (!key)
        return RES_MALLOC_FAILED;
...
static kv_item_t *kv_item_traverse(item_func func, uint8_t blk_index, const char *key)
{
    ...
    do
    {
        item = (kv_item_t *)qm_malloc(sizeof(kv_item_t));
        if (!item)
        {
            return NULL;
        }
...
static int kv_item_store(const char *key, const void *val, int len, uint16_t origin_off)
{
    ...
    store.p = (char *)qm_malloc(store.len);
    if (!store.p)
    {
        return RES_MALLOC_FAILED;
    }
...
void qm_kv_gc(void *arg)
{
    ...
    if (qm_mutex_lock(&(g_kv_mgr.kv_mutex), QM_WAIT_FOREVER) != 0)
    {
        goto exit;
    }
...
exit:
    ...
    qm_mutex_unlock(&(g_kv_mgr.kv_mutex));
    if (g_kv_mgr.gc_waiter > 0)
    {
        qm_sem_signal(&(g_kv_mgr.gc_sem));
    }
...
int qm_kv_del(const char *key)
{
    kv_item_t *item;
    int ret;
    if ((ret = qm_mutex_lock(&(g_kv_mgr.kv_mutex), qm_WAIT_FOREVER)) != RES_OK)
    {
        return ret;
    }
...
int qm_kv_del_by_prefix(const char *prefix)
{
    int i, ret;
    if ((ret = qm_mutex_lock(&(g_kv_mgr.kv_mutex), qm_WAIT_FOREVER)) != RES_OK)
    {
...
int qm_kv_set(const char *key, const void *val, int len, int sync)
{
    kv_item_t *item;
    int ret;
    ...
    {
        (g_kv_mgr.gc_waiter)++;
        qm_sem_wait(&(g_kv_mgr.gc_sem), QM_WAIT_FOREVER);
    }
...
#ifdef SUPPORT_QM_KV_LIST_CMD
static int __item_print_cb(kv_item_t *item, const char *key)
{
    char *p_key = NULL;
    char *p_val = NULL;
    p_key = (char *)qm_malloc(item->hdr.key_len + 1);
    if (!p_key)
    {
        return RES_MALLOC_FAILED;
    }
    ...
    p_val = (char *)qm_malloc(item->hdr.val_len + 1);
    if (!p_val)
    {
        qm_free(p_key);
        return RES_MALLOC_FAILED;
...
static int _kv_getx_cmd(const char *key, kv_get_type_e get_type)
{
    ...
    if (!item)
    {
        qm_cli_printf("key:%s not found\r\n", key);
        return -1;
    }

    len = item->hdr.val_len + 1;

    buffer = qm_malloc(len + 1);
    kv_item_free(item);

    if (!buffer)
    {
        qm_cli_printf("no mem\r\n");
        return -1;
    }

    memset(buffer, 0, len);

    ret = qm_kv_get(key, buffer, &len);
    if (ret != 0)
    {
        qm_cli_printf("cli: no paired kv\r\n");
    }
    else
    {
        switch (get_type)
        {
        case QM_KV_GET_TYPE_STRING:
        {
            qm_cli_printf("value is %s\r\n", buffer);
        }
        break;
        case QM_KV_GET_TYPE_BINARY:
        {
            qm_cli_printf("\r\nkv key:%s binary value is\r\n", key);

            for (index = 0; index < len; index++)
            {
                qm_cli_printf("0x%02x ", buffer[index]);
                if ((index + 1) % 16 == 0)
                {
                    qm_cli_printf("\r\n");
                }
...
static void handle_kv_cmd(char *pwbuf, int blen, int argc, char **argv)
{
    ...
    if (strcmp(rtype, "set") == 0)
    {
        ...
        ret = qm_kv_set(argv[2], argv[3], strlen(argv[3]), 1);
        if (ret != 0)
        {
           qm_cli_printf("cli set kv failed\r\n");
        }
    }
    else if (strcmp(rtype, "get") == 0)
    {
        ...
        _kv_getx_cmd(argv[2], QM_KV_GET_TYPE_STRING);
    }
    else if (strcmp(rtype, "getb") == 0)
    {
        ...
        _kv_getx_cmd(argv[2], QM_KV_GET_TYPE_BINARY);
    }
    else if (strcmp(rtype, "del") == 0)
    {
        ...
        ret = qm_kv_del(argv[2]);
        if (ret != 0)
        {
            qm_cli_printf("cli kv del failed\r\n");
        }
#ifdef SUPPORT_QM_KV_LIST_CMD
    }
    ...
    {
        qm_cli_printf("\"kv %s\" not support!\r\n", rtype);
    }
...
static struct cli_command ncmd = {
    "qm kv",
#ifdef CONFIG_QM_KV_LIST_CMD
    "qm kv [set key value | get key | getb key | del key | list | clear]",
#else
    "qm kv [set key value | get key | getb key | del key | clear]",
#endif
    handle_kv_cmd};
#endif
...
int qm_kv_init(void)
{
    ...
    if ((ret = qm_mutex_new(&(g_kv_mgr.kv_mutex))) != 0)
    {
        return ret;
    }

#ifdef CONFIG_QM_CLI
    qm_cli_register_command(&ncmd);
#endif
    ...
    if ((ret = qm_sem_new(&(g_kv_mgr.gc_sem), 0)) != RES_OK)
    {
        return ret;
    }
...
void qm_kv_deinit(void)
{
    g_kv_mgr.kv_initialize = 0;
    qm_sem_free(&(g_kv_mgr.gc_sem));
    qm_mutex_free(&(g_kv_mgr.kv_mutex));
}
