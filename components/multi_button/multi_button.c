...
#define EVENT_CB(ev)    \
    if (handle->cb[ev]) \
    handle->cb[ev]((multi_button_t *)handle)

//button handle list head.
static multi_button_t *head_handle = NULL;
...
{
    memset(handle, 0, sizeof(multi_button_t));
...
/**
  ...
  * @retval None
  */
static void button_handler(multi_button_t *handle)
{
    ...
    { //not equal to prev one
        ...
        if (++(handle->debounce_cnt) >= CONFIG_MULTI_BUTTON_DEBOUNCE_TICKS)
        {
    ...
    case 1:
        ...
        else if (handle->ticks > CONFIG_MULTI_BUTTON_LONG_TICKS)
        {
    ...
    case 2:
        ...
        else if (handle->ticks > CONFIG_MULTI_BUTTON_SHORT_TICKS)
        { //released timeout
            if (handle->repeat == 1)
            {
    ...
    case 3:
        ...
        { //released press up
            handle->event = (uint8_t)PRESS_EVENT_UP;
            EVENT_CB(PRESS_EVENT_UP);
            if (handle->ticks < CONFIG_MULTI_BUTTON_SHORT_TICKS)
            {
                handle->ticks = 0;
...
{
    multi_button_t *target = head_handle;
    while (target)
    {
        if (target == handle)
