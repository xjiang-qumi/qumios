...
#ifndef CONFIG_DNS_SERVER_PRINT
#define CONFIG_DNS_SERVER_PRINT         0
#endif

#ifndef CONFIG_DNS_SERVER_BUF_SIZE
#define CONFIG_DNS_SERVER_BUF_SIZE      128
#endif

#ifndef CONFIG_DNS_SERVER_TASK_SIZE  
#define CONFIG_DNS_SERVER_TASK_SIZE     4096
#endif

#ifndef CONFIG_DNS_SERVER_TASK_PRIO  
#define CONFIG_DNS_SERVER_TASK_PRIO     20
#endif

#ifndef CONFIG_DNS_DOMAIN_MAX_LEN  
#define CONFIG_DNS_DOMAIN_MAX_LEN       64
#endif

#ifndef CONFIG_DNS_IP_MAX_LEN  
#define CONFIG_DNS_IP_MAX_LEN          16
#endif

typedef struct {
    uint8_t addr[4];
    char domain[CONFIG_DNS_DOMAIN_MAX_LEN + 1];
}domain_info_t;
...
static dns_server_ctx_t g_dns_server_ctx = {0};
...
{
    if (strlen(g_dns_server_ctx.domain_info.domain) == strlen(domain_name) &&
        strcmp(g_dns_server_ctx.domain_info.domain, domain_name) == 0){
        addr[0] = g_dns_server_ctx.domain_info.addr[0];
        addr[1] = g_dns_server_ctx.domain_info.addr[1];
        addr[2] = g_dns_server_ctx.domain_info.addr[2];
        addr[3] = g_dns_server_ctx.domain_info.addr[3];
        return 0;
    }else{
        return -1;
    }
...
void print_resource_record(struct ResourceRecord *rr)
{
    ...
    while (rr)
    {
        QM_LOGD(LOG_TAG, "  ResourceRecord { name '%s', type %u, class %u, ttl %u, rd_length %u, ",
               rr->name,
               rr->type,
               rr->class,
               rr->ttl,
        ...
        case A_Resource_RecordType:
            QM_LOGD(LOG_TAG, "Address Resource Record { address ");
        ...
        case NS_Resource_RecordType:
            QM_LOGD(LOG_TAG, "Name Server Resource Record { name %s }",
                   rd->name_server_record.name);
            break;
        case CNAME_Resource_RecordType:
            QM_LOGD(LOG_TAG, "Canonical Name Resource Record { name %u }",
                   rd->cname_record.name);
            break;
        case SOA_Resource_RecordType:
            QM_LOGD(LOG_TAG, "SOA { MName '%s', RName '%s', serial %u, refresh %u, retry %u, expire %u, minimum %u }",
                   rd->soa_record.MName,
                   rd->soa_record.RName,
        ...
        case MX_Resource_RecordType:
            QM_LOGD(LOG_TAG, "Mail Exchange Record { preference %u, exchange '%s' }",
                   rd->mx_record.preference,
                   rd->mx_record.exchange);
            break;
        case TXT_Resource_RecordType:
            QM_LOGD(LOG_TAG, "Text Resource Record { txt_data '%s' }",
                   rd->txt_record.txt_data);
            break;
        case AAAA_Resource_RecordType:
            QM_LOGD(LOG_TAG, "AAAA Resource Record { address ");
...
{
    char name[CONFIG_DNS_DOMAIN_MAX_LEN + 1];
...
int decode_msg(struct Message *msg, const uint8_t *buffer, int size)
{
    ...
    {
        QM_LOGE(LOG_TAG, "Only questions expected!\n");
        return -1;
    }
    ...
    for (i = 0; i < qcount; ++i)
    {
        struct Question *q = qm_malloc(sizeof(struct Question));
...
void resolver_process(struct Message *msg)
{
    ...
    while (q)
    {
        rr = qm_malloc(sizeof(struct ResourceRecord));
        ...
        */
        default:
            ...
            QM_LOGE(LOG_TAG, "Cannot answer question of type %d.\n", q->qType);
            goto next;
        }
...
int encode_resource_records(struct ResourceRecord *rr, uint8_t **buffer)
{
    ...
    while (rr)
    {
        ...
        case AAAA_Resource_RecordType:
            ...
            break;
        default:
            QM_LOGD(LOG_TAG, "Unknown type %u. => Ignore resource record.", rr->type);
            return 1;
        }
...
void qm_free_resource_records(struct ResourceRecord *rr)
{
...
void qm_free_questions(struct Question *qq)
{
...
static void dns_server_task(void *arg)
{
    ...
    uint8_t buffer[DNS_SERVER_BUF_SIZE] = {0};
    int nbytes = 0;
    int port = 53;
    int socket = -1;
    ...
    dns_server_ctx_t *dns_server_ctx = (dns_server_ctx_t*)arg;

    socket = qm_udp_server_create(NULL, port);
    if(socket < 0){
        QM_LOGE(LOG_TAG, "udp server creat error");
        goto __exit;
    }

    while (!dns_server_ctx->exit)
    {
        qm_free_questions(msg.questions);
        qm_free_resource_records(msg.answers);
        qm_free_resource_records(msg.authorities);
        qm_free_resource_records(msg.additionals);
        memset(&msg, 0, sizeof(struct Message));

        nbytes = qm_udp_read(socket, (char*)buffer, sizeof(buffer));
        ...
        int buflen = p - buffer;
        qm_udp_write(socket, (char*)buffer, buflen, 0);
    }

    qm_udp_close(socket);

__exit:
    qm_task_exit(&dns_server_ctx->task);
}
...
int dns_server_ip_register(const char *domain, const char *ip)
{
    ...
    if(strlen(domain) > CONFIG_DNS_DOMAIN_MAX_LEN){
        return -QM_EINVAL;
    }

    if(strlen(ip) > CONFIG_DNS_IP_MAX_LEN){
        return -QM_EINVAL;
    }

    sscanf(ip, "%d.%d.%d.%d", addr, addr + 1, addr + 2, addr + 3);
    g_dns_server_ctx.domain_info.addr[0] = (uint8_t)addr[0];
    g_dns_server_ctx.domain_info.addr[1] = (uint8_t)addr[1];
    g_dns_server_ctx.domain_info.addr[2] = (uint8_t)addr[2];
    g_dns_server_ctx.domain_info.addr[3] = (uint8_t)addr[3];
    memset(g_dns_server_ctx.domain_info.domain, 0, sizeof(g_dns_server_ctx.domain_info.domain));
    strcpy(g_dns_server_ctx.domain_info.domain, domain);
...
{
    g_dns_server_ctx.exit = 0;
    memset(&g_dns_server_ctx.task, 0, sizeof(g_dns_server_ctx.task));
    return qm_task_new(&g_dns_server_ctx.task, "dns server", dns_server_task, &g_dns_server_ctx, CONFIG_DNS_SERVER_TASK_SIZE, CONFIG_DNS_SERVER_TASK_PRIO);
}

int dns_server_stop(void)
{
    g_dns_server_ctx.exit = 1;
    return QM_EOK;
}
